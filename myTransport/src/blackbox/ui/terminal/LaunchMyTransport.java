...
public class LaunchMyTransport {
	public static void main(String[] args) {
		...
		try {
			...
			MyTransport myTransport = new MyTransport(ip, Integer
					.parseInt(args[0]));
			...
			IHM: while (true) {
				...
				if (subMenu) {
					try {
						...
						case 1:
							System.out
									.print("\tchoose a port number to start MyConcert: ");
							...
							MyConcert myConcert = new MyConcert(ip, 0,
									myTransport);
						...
						case 2:
							System.out
									.print("\tchoose a port number to start MyFoot: ");
							port = Integer.parseInt(input.readLine());
				...
				if (!subMenu) {
					...
					try {
						...
						case 3:
						case 4:
							if (myTransport.getNetworks().size() > chx - 2
									&& myTransport.getNetworks().get(chx - 2)
											.getIdentifier().equals("Foot")) {
								System.out
										.println("\t1) Publish a football match");
								System.out
										.println("\t2) Search a football match");
								System.out.print("\t---> ");
								chx = Integer.parseInt(input.readLine().trim());
								switch (chx) {
								case 1:
									...
									if (!key2.equals(""))
										myTransport.put(key + "+" + key2,
												message + "+" + message2
														+ "+car, maybe...");
									else
										myTransport.put(key, message + "+"
												+ message2 + "+car, maybe...");
									break;
								case 2:
									System.out.print("\n\tDay = ");
							...
							} else if (myTransport.getNetworks().size() > chx - 2
									&& myTransport.getNetworks().get(chx - 2)
											.getIdentifier().equals("Concert")) {
								...
								case 1:
									...
									if (!key2.equals(""))
										...
										myTransport.put(key, "myTransport+"
												+ message + "+car, maybe...");
									break;
								case 2:
									System.out.print("\n\tDay = ");
